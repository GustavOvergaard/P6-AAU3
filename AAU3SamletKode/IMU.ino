...
float komplementaerFilter() {
  omega_r = (GYRO_Read(MPU_addr1)+GYRO_Read(MPU_addr2)) / 2;
  acc = (ACC_Read(MPU_addr1) - AccOffset1 +ACC_Read(MPU_addr2) - AccOffset2) / 2;

  float vinkel = 0;
  float samplingperiod_s = Samplingstid / 1000.0;

  vinkel = 0.98 * (senesteVinkel + (omega_r * samplingperiod_s)) + 0.02 * acc;
  senesteVinkel = vinkel;
