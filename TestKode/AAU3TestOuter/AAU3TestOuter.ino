...
void Sampling(void *pvParameters ) {
  ...
  for (;;) {
    vTaskDelayUntil(&xLastWakeTime, xPeriod);
    //Serial.print("samp: "); Serial.println(state);
    ...
    if (state == 0 and recovery == 0) {
      //theta_meas = complementaryFilter2() - 0.0749;
      theta_meas = potmeter() - potOffset;
      //Serial.println(theta_meas);
      //Serial.println(potmeter() - potOffset);

      //lav moving average
      avg_theta_rf = avg_theta_rf + theta_meas / avgOver - avg_theta_rf / avgOver;
      theta_rf = theta_meas - avg_theta_rf;

      Serial.print(complementaryFilter2() - 0.0749, 5); Serial.print(",");; Serial.print(current, 5); Serial.print(","); Serial.println(kont);
    }
...
void printTask(void *pvParameters ) {
  ...
  TickType_t xLastWakeTime;
  const TickType_t xPeriod = 100; //samples 10ms
  xLastWakeTime = xTaskGetTickCount();
  for (;;) {
    vTaskDelayUntil(&xLastWakeTime, xPeriod);
    if (state == 0 and recovery == 0) {
